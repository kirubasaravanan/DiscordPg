...
Status: All 7 phases implemented — database (`backend/`), backend API, dashboard (`dashboard/`), Discord bot (`discord_bot/`), AI service (`ai_engine/`), and production packaging (`docker/`, root `docker-compose.yml`). This was originally a Phase 1 (Architecture-only) document; §4.6, §7, §9, §12, and §13 below now also record where implementation confirmed or corrected the original design, and §12 documents every infrastructure gap hit along the way (object storage, Discord, Ollama, Docker Hub — all specific to this sandboxed environment, not to real-world use) — see [DATABASE.md](DATABASE.md), [API.md](API.md), [AI_DESIGN.md](AI_DESIGN.md), and [DEPLOYMENT.md](DEPLOYMENT.md) for the fuller implementation notes.
...
5. **Document uploads use pre-signed URLs** rather than the backend proxying file bytes. *(Implemented in Phase 3c — see items 15-16 below for exactly what was and wasn't verified about it.)*
...
32. **This environment's outbound network policy also blocks Docker Hub's image-blob CDN (`production.cloudfront.docker.com`)** — a fourth instance of the same category of gap as items 15 (S3/R2), 25 (Discord), and 27 (Ollama/HuggingFace), confirmed the same way: the proxy's own connection-failure log, not assumed, and confirmed *not* fixable by `--network host` (tried directly — the block sits at the daemon's own image-pull layer, unrelated to a container's network namespace). Unlike items 15/25/27, this gap is specific to *this sandboxed agent environment*, not to real-world deployment or CI — Docker Hub is about as universally reachable as a container registry gets. What **is** verified here regardless: `docker compose config` fully resolves every service, environment-variable interpolation, healthcheck, volume, and `depends_on` condition in `docker-compose.yml`; `docker compose build` parses every Dockerfile correctly and fails at exactly (and only) the blocked base-image pull, confirmed by reading the actual error output, not assumed. See [DEPLOYMENT.md](DEPLOYMENT.md) §6.
33. **Proceeded directly with "build now, verify later" for the Phase 7 Docker Hub gap without a fourth `AskUserQuestion` round.** Items 15 (S3/R2 credentials, Phase 3c), 25 (Discord bot token, Phase 5), and 27 (Ollama access, Phase 6) each asked first and got the same answer — build the real thing, document exactly what's unverified, don't stub or wait. A fourth identical question about a gap that's *narrower in consequence* than the prior three (this one doesn't affect real users or CI, only this sandbox's own ability to self-test) would have been repetitive rather than genuinely open. If this judgment turns out wrong, it's a one-line revert — nothing here forecloses any other option.
34. **`backend`'s and `discord_bot`'s `docker-compose.yml` restart policies are deliberately different.** `backend` gets `restart: on-failure` — a transient database-connection race at startup (e.g. Postgres's healthcheck passing before it's fully ready for the backend's specific connection pattern) is worth a silent retry. `discord_bot` gets no restart policy at all — a missing or invalid `DISCORD_BOT_TOKEN` is a configuration problem, and `discord_bot/main.py` already exits with a clear message rather than retrying (Phase 5, see item 25); crash-looping a config error in Compose would hide the same failure `docker compose ps` should show plainly. `dashboard` and `ai_engine` likewise get no restart policy, for the same "config errors should fail loudly" reasoning — neither has a plausible transient-failure mode analogous to backend's DB-connection race.
35. **A single root `.env` serves all 4 services via `env_file:` plus per-service `environment:` overrides, relying on each service's own `Settings` already having `extra="ignore"`** (true since each service's config was first written) rather than requiring 4 separate `.env` files inside a Compose deployment. The per-service `environment:` blocks override exactly the keys that must differ between "run directly with `uv run`" (localhost-oriented defaults) and "run inside Compose" (container-network hostnames: `postgres`, `ai_engine`, `backend`) — `DATABASE_URL`, `AI_ENGINE_URL`, `API_BASE_URL`, `STORAGE_LOCAL_BASE_URL`, `OLLAMA_HOST`. Confirmed by reading `docker compose config`'s fully-resolved output, not just the unresolved YAML.
36. **`docker/backend.Dockerfile` inlines its migrate-then-serve command directly in `CMD` rather than using a separate entrypoint script.** A first draft tried `COPY --chown=appuser:appuser ../docker/backend-entrypoint.sh /app/entrypoint.sh` from a Dockerfile whose build context is `backend/` — Docker rejects any `COPY` source outside the build context, regardless of chown flags or relative-path tricks. Fixed by dropping the separate script file and inlining the same logic as `CMD ["sh", "-c", "alembic upgrade head && exec uvicorn app.main:app --host 0.0.0.0 --port 8000"]` — one line, no extra file, identical behavior.
