...
**The delivery mechanism these jobs will notify through is implemented (Phase 5)**: `app/services/notification_service.py` sends outbound Discord messages — `send_discord_dm(discord_id, message)` for per-tenant notifications, `send_channel_message(channel_id, message)` for the management report — via Discord's plain REST API, using the same bot token as `discord_bot/`. It does **not** go through the interactive bot process: sending a message never needs a gateway connection, only *receiving* interactions does, and the bot is a separate long-lived process anyway. **Wiring the three jobs above to actually run on a schedule and call this is Phase 6's job**, not Phase 5's ([ROADMAP.md](ROADMAP.md) Phase 6 — the management/income report jobs need the AI summary generator, which doesn't exist yet). Unverified against a live Discord account — see §12 item 25.
